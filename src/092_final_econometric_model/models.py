...
class FinalEconometricModels:
    ...
    def run_price_channel_lp_irf(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        模型：P^lng_{i,t+h} = β_h·us_prod_shock_t + θ_h·(us_prod_shock_t × ovi_gas_{i,t-1}) + 
                              δ_h·(us_prod_shock_t × distance_to_us_i) + Γ·Controls + α_i + λ_t + η_{i,t+h}
        
        核心关注：θ_h 系数，预期为负值（OVI缓冲价格冲击）
        ...
        try:
            ...
            # 准备解释变量
            base_vars = ['us_prod_shock', 'shock_ovi_interaction']
    ...
    def run_quantity_channel_lp_irf(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        模型：g_{i,t+h} = β_h·us_prod_shock_t + θ_h·(us_prod_shock_t × ovi_gas_{i,t-1}) + 
                         δ_h·(us_prod_shock_t × distance_to_us_i) + Γ·Controls + α_i + λ_t + η_{i,t+h}
        
        核心关注：θ_h 系数，预期为负值（OVI赋予主动调节能力）
    ...
    def generate_irf_plots(self, price_results: Dict, quantity_results: Dict) -> None:
        """
        生成脉冲响应函数图表
        ...
        try:
            ...
            if price_results.get('status') == 'success' and 'horizon_results' in price_results:
                ...
                ax1.plot(price_horizons, price_coefs, 'o-', color='#2E8B57', linewidth=3, 
                        markersize=8, label='θ_h (OVI×冲击交互项)', markerfacecolor='white', 
                        markeredgewidth=2)
                ...
                ax1.text(0.5, 0.5, '价格通道数据不可用', ha='center', va='center', 
            ...
            if quantity_results.get('status') == 'success' and 'horizon_results' in quantity_results:
                ...
                ax2.text(0.5, 0.5, '数量通道数据不可用', ha='center', va='center',
            ...
            output_file = figure_path / "final_lp_irf_results.png"
    ...
    def run_final_analysis(self, df: pd.DataFrame) -> Dict[str, Any]:
        """
        运行完整的最终分析
        ...
        # 生成图表
        logger.info("\n" + "="*50)
        self.generate_irf_plots(price_results, quantity_results)
        
        # 汇总结果
        final_results = {
            'analysis_type': '092_final_econometric_model',
            'models': {
    ...
    def _save_results(self, results: Dict) -> None:
        ...
        try:
            ...
            # 保存JSON结果
            output_file = self.output_dir / "final_analysis_results.json"
