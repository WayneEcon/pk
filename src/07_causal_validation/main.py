...
class CausalValidationPipeline:
    ...
    def load_data_from_modules(self) -> tuple[Dict[int, nx.Graph], pd.DataFrame]:
        ...
        # 尝试从05模块加载DLI数据  
        dli_data = self._load_dli_from_05()
    ...
    def _load_dli_from_05(self) -> pd.DataFrame:
        """从05模块加载DLI数据"""
        try:
            # 使用标准数据接口
            sys.path.append(str(project_root / "05_dli_analysis"))
            from data_interface import get_dli_panel_data
            return get_dli_panel_data()
        except:
            logger.warning("⚠️ 无法从05模块导入，尝试文件加载...")
